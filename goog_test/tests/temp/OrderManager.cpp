...
bool OrderManager::handle_order(){
    ...
    ExecutionOrder* exec_order = new ExecutionOrder(e);
    strategy_to_ordermanager.pop();
    exec_order->setState(state_);
    if (exec_order->getState() == orderstate::REJECTED){
        ordermanager_to_strategy.push(*exec_order);
        return true;
    }
    else {
        list_orders[e.getID()] = *exec_order;
        order=e;
